...
GUIDE = '''
Guide for Constructing MySQL Queries for Compounds and Biological Activities
General Best Practices
Understand Schema Relationships:

Use foreign key relations to join tables accurately (e.g., MOLREGNO links compound-related tables; ACTIVITY_ID connects activities and their properties).
Refer to CHEMBL_ID_LOOKUP to ensure uniform identification across entities.
Account for Data Issues:

Handle duplicates using DISTINCT or filtering by POTENTIAL_DUPLICATE = 0.
Account for synonyms or alternate names in MOLECULE_SYNONYMS.
Use DATA_VALIDITY_COMMENT to filter out unreliable activity data (e.g., exclude rows flagged as Potential author error or Non-standard unit type).
Use Standardized Values:

Prefer STANDARD_VALUE and STANDARD_UNITS from the ACTIVITIES table for consistency.
Validate STANDARD_TYPE in ACTIVITY_STDS_LOOKUP to ensure values are within expected ranges.
Filter and Aggregate Data:

Use groupings to summarize data (e.g., average STANDARD_VALUE for a compound across assays).
Include compound properties from COMPOUND_PROPERTIES to contextualize activity data.
Optimize Performance:

Use indexed columns like primary keys (ACTIVITY_ID, MOLREGNO) in joins.
Limit result sets early in queries with WHERE clauses.
Query Templates
1. Retrieve Activities for a Compound
SELECT 
    a.ACTIVITY_ID, 
    c.PREF_NAME AS CompoundName, 
    a.STANDARD_TYPE, 
    a.STANDARD_VALUE, 
    a.STANDARD_UNITS, 
...
FROM 
    ACTIVITIES a
JOIN 
    MOLECULE_DICTIONARY c ON a.MOLREGNO = c.MOLREGNO
WHERE 
    c.PREF_NAME = 'CompoundName'
    AND a.POTENTIAL_DUPLICATE = 0
    AND (a.DATA_VALIDITY_COMMENT IS NULL OR a.DATA_VALIDITY_COMMENT = 'Manually validated');
2. Identify Related Assays for a Compound
sql
Copy
Edit
SELECT 
    a.ACTIVITY_ID, 
    ass.DESCRIPTION AS AssayDescription, 
    ass.ASSAY_TYPE, 
    ass.ASSAY_CATEGORY
FROM 
    ACTIVITIES a
...
WHERE 
    a.MOLREGNO = (SELECT MOLREGNO FROM MOLECULE_DICTIONARY WHERE PREF_NAME = 'CompoundName');
3. Retrieve Compound Synonyms and Properties
sql
Copy
Edit
SELECT 
    ...
    cp.PSA
FROM 
    MOLECULE_DICTIONARY c
LEFT JOIN 
    MOLECULE_SYNONYMS s ON c.MOLREGNO = s.MOLREGNO
LEFT JOIN 
    COMPOUND_PROPERTIES cp ON c.MOLREGNO = cp.MOLREGNO
WHERE 
    c.PREF_NAME = 'CompoundName';
4. Aggregate Biological Activity for a Compound
sql
Copy
Edit
SELECT 
    c.PREF_NAME AS CompoundName, 
    a.STANDARD_TYPE, 
    AVG(a.STANDARD_VALUE) AS AvgActivityValue, 
    a.STANDARD_UNITS
FROM 
    ACTIVITIES a
JOIN 
...
WHERE 
    ...
    AND (a.DATA_VALIDITY_COMMENT IS NULL OR a.DATA_VALIDITY_COMMENT = 'Manually validated')
GROUP BY 
    c.PREF_NAME, a.STANDARD_TYPE, a.STANDARD_UNITS;
5. Link Mechanism of Action to Activities
sql
Copy
Edit
SELECT 
...
FROM 
    ACTIVITIES a
JOIN 
    DRUG_MECHANISM dm ON a.MOLREGNO = dm.MOLREGNO
JOIN 
    MOLECULE_DICTIONARY c ON a.MOLREGNO = c.MOLREGNO
WHERE 
    c.PREF_NAME = 'CompoundName';
Common Error Handling
Duplicate Data: Use POTENTIAL_DUPLICATE and DISTINCT to refine results.
Non-standard Units: Validate and normalize units using ACTIVITY_STDS_LOOKUP.
Invalid or Out-of-Range Values: Exclude rows flagged in DATA_VALIDITY_COMMENT.
This guide helps construct reliable MySQL queries to explore compounds and their biological activities within ChEMBL data, accounting for common issues like duplicates, inconsistent naming, and invalid data.
'''